...
report = await EvalRunner(app, judge=judge, judge_model="...").run("evals/")
assert report.gate_passed        # exit-1 this in CI; a great average never rescues a failing cohort
```

The `handoff` grader passes when the session escalated to a human, so "hands off on hardship" is testable end-to-end. The judge grades with the full context the agent had, its system prompt and the whole conversation, states its reasoning before the verdict (kept in the report as `judge_output`), and defaults to strict: any doubt fails, the right tuning for compliance-critical cohorts; use `strictness: lenient` for tone/style cohorts where over-flagging suppresses quality. Use a judge from a **different model family** than the agent, same-family judges share the agent's blind spots, and the runner warns if the models match.
...
Storage sits behind the four-method `AuditStore` protocol (SQLite default, `InMemoryAuditStore` reference). For production retention, a `PostgresAuditStore` ships in the box, same chain logic over one table, appends serialized across app instances by a transaction-scoped advisory lock:

```python
from zolva import AuditLog, PostgresAuditStore
...
### Compliance, EU AI Act evidence on demand

The artifacts Zolva already produces map onto the controls a bank must evidence. `zolva compliance` reads them back and emits one signed bundle mapping the hash-chained audit (Art. 12 record-keeping), the config hashes (Art. 13 traceable system definition), handover events (Art. 14 human oversight), and eval gates plus the SARR scorecard (Art. 15 / SR 11-7 accuracy and ongoing monitoring) to named articles:

```bash
zolva compliance audit.sqlite --agents agents/ --eval-report eval.json --out pack.json --gate
```

The audit chain's head hash is the tamper-evidence anchor; `bundle_sha256` self-seals the report; `--gate` exits 1 unless every control passes. This is packaging of existing evidence, not a compliance guarantee, the bank's own mapping and sign-off still decide adequacy.

```python
from zolva import AuditLog, build_report
report = build_report(AuditLog("audit.sqlite"), agents=agents, eval_report=eval_report)
assert report.regulator_ready
```
...
OTelExporter().attach(app)     # pip install "zolva[otel]"
```
